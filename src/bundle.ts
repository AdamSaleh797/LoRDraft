...
type callback_fn = (err: Error|null) => void;
...
function unzip_file(
    file: string, output_dir: string, callback: callback_fn = () => undefined) {
  child_process.exec(
      `unzip -u ${file} -d ${output_dir}`, {'cwd': _ASSET_DIR},
      (err) => callback(err));
}

export function downloadZipAsset(
    url: string, dst_file_name: string,
...
export function extractFromBundle(
    bundle: string, rel_path: string, name: string,
    callback: callback_fn = () => undefined) {
  prepareAssetsDir();

  fs.cp(
      path.join(_ASSET_DIR, bundle, rel_path), path.join(_ASSET_DIR, name),
      (err) => {
        callback(err);
      });
}
...
export function removeBundle(
    bundle: string, callback: callback_fn = () => undefined) {
  prepareAssetsDir();
